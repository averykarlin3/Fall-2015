...
\item The FIFO acts as a file, with permissions inputted, blocking on open (unable to be written or read) until a connection is established on both ends
\item The FIFO is not opened immediately after created, and is used otherwise the same as any other file
\end{enumerate}
\end{enumerate}
...
\item After the connection, the private pipe is removed by the subserver, which then ends, and the main server makes a new well-known pipe
\end{enumerate}
\end{enumerate}
