...
\end{enumerate}
\item wait(\&status) - Waits in the parent process until at least one child process has returned information, setting the ``int status'' variable, which must have been declared
\begin{enumerate}
\item status stores a return value as the first byte, followed by other info, such that WEXITSTATUS(status) gives only the return value
