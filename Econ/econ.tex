...
\begin{enumerate}
\item Since foreign currency is needed to purchase both goods, services, and assets (as well as factor income and monetary transfers, assumed for simplicity)
\item Thus, the quantity demanded of domestic is equal to that demanded of foreign, preserved by the exchange rate, such that circular international flow is valid at equilibrium
\item For any change in one account, the change in the exchange rate resulting causes the opposite change in the other account
\end{enumerate}
...
\begin{enumerate}
\item Purchasing power parity is almost always different from nominal exchange rate, partially due to poor countries having lower price levels, but in the long run, they are generally good indicators of average nominal interest rates level
\item It is also very stable in the long run, just like the real exchange rate is, theoretically the same assuming no trade barriers
\end{enumerate}
\end{enumerate}
