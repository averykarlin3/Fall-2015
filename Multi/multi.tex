...
The Sunrise Lemma states for any sequence $(a_n)^\infty_{n=1}$ in $\mathbb{R}$, $\exists$ monotone subsequence $(a_{n_k})^\infty_{k=1}$, where $(n_k)^\infty_{k=1}$ is a strictly increasing sequence in $\mathbb{N}$ and $n_k \geq k$ for all $k \in \mathbb{N}$.
...
\section{Extreme Value Theorem NEXT}
...
Lets assume that f is not bounded above. The sequence, $x_n \in [a, b]$ is defined for all $n \in \mathbb{N}$, we can state by the assumption that $\exists n$, such that $f(x_n) > n$.
