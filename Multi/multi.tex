...
The Cartesian Plane represents the set $\varmathbb{R}^2 := \{(x, y)\+|\+x, y\in \varmathbb(R)\}$. This is known as the \textbf{Cartesian Product} of $\varmathbb{R}$ with
...
$dist(u, v\+;\+ a, b) < \varepsilon$. Since the definition is symmetrical, $\partial D^c = \partial D$.
...
For our purpose, we define the distance function in 2D as:
$$dist(x, y\+\+;\+a,b) := \sqrt{(x - a)^2 + (y - b)^2}$$
We use the Euclidean distance because it is preserved under many
transformations.
...
Functions are defined as one-to-one if for f(x, y), $(x, y), (u, v) \in D, f(x, y) = f(u, v)$ iff x = u $\wedge$ (and) y = v. 
