...
This is known as the \underline{Euclidean Distance}. It is used because it is
preserved under many transformations.
...
\begin{enumerate}
    ...
    \item \textbf{Triangle Inequality}: $dist(\vec{p}, \vec{q}) + dist(\vec{q}, \vec{r}) \geq dist(\vec{p},
        \vec{r})$
    \begin{itemize}
        \item This can be generalized by mathematical induction to
            $dist(\vec{p_0}, \vec{q_n}) \leq \sum_{j = 1}^n
            dist(\vec{p}_{j -
        1}, \vec{p}_{j})$ (Otherwise known that the shortest distance
            between two points is the straight line, or the
            \textbf{Generalized Triangle Inequality} or the ``Broken Line
            Inequality'')
    \end{itemize}
\end{enumerate}
...
\textbf{Cor}: Coordinatewise Nature of Convergence
